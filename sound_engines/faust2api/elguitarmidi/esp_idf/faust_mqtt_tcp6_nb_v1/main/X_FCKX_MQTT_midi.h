#define MQTT_MIDICTRL 1

#define ACTIVE 0

/************************** BEGIN fckx-mqtt_midi.h **************************/
/************************************************************************
Midi over MQTT implementation

This code is generated by FCKX and is based/inspired by esp32-midi.h, a FAUST Architecture File 
included in DspFaust.cpp generated with faust2api -esp32

This code contains only the transport interfaces. The UART dependent parts are converted to MQTT  
 ************************************************************************/

#ifndef __fckx_mqtt_midi__
#define __fckx_mqtt_midi__

#define FCKX_MQTT_MIDICTRL 1
#include <cstdlib>

#if !FCKX_MQTT_MIDICTRL
...
#define RX_BUF_SIZE 1024    //MQTT implementation might need a similar sized buffer
                            //to store messages after conversion to binary
...
#if FCKX_MQTT_MIDICTRL
/*
Handler for single midi message
*/

#if ACTIVE
void process_single_midi( void* message) {
...
/**
 * MQTT based MIDI handler for the ESP32 boards.
 */
class esp32_midi : public midi_handler {
        ...
        void processMidi()
        {
            ...
            while (true) {
                ...
                //int rxBytes = uart_read_bytes(UART_NUM_1, data, RX_BUF_SIZE, 1);
                 ...
                 /**
                 * @brief read single midi message (bytes) from MQTT message
                 *
                 * @param msg pointer to the mqtt message that contains a single MIDI message.
                 * @param buf     pointer to the buffer.
                 * @param length  data length
                 * @param ticks_to_wait sTimeout, count in RTOS ticks
                 *
                 * @return
                 *     - (-1) Error
                 *     - OTHERS (>=0) The number of bytes read from MQTT message
                 */               
                int rxBytes = read_single_midimsg_bytes(mqttmsg, data,RX_BUFF_SIZE, 1);
                if (rxBytes > 0) {
                    for (int i = 0; i < rxBytes; i++) {
                        if (parser.Parse((uchar)data[i], &message)) {
                            unsigned char status = message.GetStatus();
                 ...
                 /**
                 * @brief read sequence of midi messages (bytes) from MQTT message
                 *
                 * @param msg pointer to the mqtt message that contains a a sequence of MIDI message.
                 * @param buf     pointer to the buffer.
                 * @param length  data length
                 * @param ticks_to_wait sTimeout, count in RTOS ticks
                 *
                 ...
                 *     - OTHERS (>=0) The number of bytes read from MQTT message
                 */               
               // int rxBytes = read_multi_midimsg_bytes(mqttmsg, data,RX_BUFF_SIZE, 1);
/*
* Look for some inspiration in:
* https://ccrma.stanford.edu/~rmichon/faustTutorials/#making-a-midi-polyphonic-synthesizer
*/  
        static void processMidiHandler(void* arg)
        {
            static_cast<esp32_midi*>(arg)->processMidi();
