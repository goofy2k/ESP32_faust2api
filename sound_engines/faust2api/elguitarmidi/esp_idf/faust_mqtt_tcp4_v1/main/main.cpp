...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event){  
   ...
   //result =  strncmp(event->topic, "/faust/api2/gate",event->topic_len); //OK
    ...
    if (strncmp(event->topic, "/faust/api2/gate",event->topic_len) == 0) {printf("HIT result=%s\n"," /faust/api2/gate");}
      else  if (strncmp(event->topic, "/faust/api",event->topic_len) == 0) {printf("HIT result=%s\n"," /faust/api");};
...
while(1) {
        ...
        msg_id = esp_mqtt_client_publish(mqtt_client, "/faust/jsonui", DSP->getJSONUI(), 0, 0, 0);
